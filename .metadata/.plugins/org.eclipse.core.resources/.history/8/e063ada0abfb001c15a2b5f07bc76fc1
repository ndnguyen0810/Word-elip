...
//xu ly dư liệu
	...
	public void Update(LoaiThucAn l) {
		try {
			String sql = String.format("update loaita set tenloai='%s' where maloai=%d", l.getTenLoai(), l.getMaLoai());
			PreparedStatement s = con.prepareStatement(sql);
			if (s.executeUpdate() > 0) {
	...
	public void Delete(LoaiThucAn l) {
		try {
			String sql = String.format("delete from loaita where maloai=%d", l.getMaLoai());
			PreparedStatement s = con.prepareStatement(sql);
			if (s.executeUpdate() > 0) {
